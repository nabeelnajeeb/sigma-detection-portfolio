...
**Dataset evidence:** 360 hits in the initial smoke test using SigmaHQ's community rule.
...
Logic: flag execution of `PSEXESVC.exe`, or either of two well-documented tools in the same family of `PAExec.exe` (an open-source PsExec clone) and `RemComSvc.exe` (another open-source remote-execution tool, also used directly by CrackMapExec's `smbexec` module).
